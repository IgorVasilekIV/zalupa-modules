"""
😘 fork info: add close button instead info, premium emoji for inline bot trough extera feature
"""
...
class ReadFileMod(loader.Module):
    ...
    async def _show_page(self, msg_or_call, index):
        ...
        text = f'<a href="tg://emoji?id=5461114598344129990">📒</a> Страница {index + 1}/{total}\n<pre>{utils.escape_html(self.chunks[index])}</pre>'
    ...
    async def _close_cb(self, call):
        ...
        if self.file_path and os.path.exists(self.file_path):
            try:
                ...
                await call.answer(f"Ошибка при удалении файла: {e}", show_alert=True)
